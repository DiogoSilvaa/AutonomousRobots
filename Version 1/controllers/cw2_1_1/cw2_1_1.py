...
# Compared to the trace_maker controller, this controller has an additional strophotaxis behaviour.
...
THRESH_DURATION = 34 #variable to hold the maximum duration of each strophotaxis cycle
...
"""Default starting side is left side. (SIDE_LEFT)"""
turning_side = 0
stro_cycle_change = 0 #CYCLE_CHANGE_INACTIVE
    ...
    if gs_val[side] > THRESH_WHITE:
        ...
        if stro_cycle_change == CYCLE_CHANGE_ACTIVE:
            if side==SIDE_LEFT:
                ...
                ret_l = MOTOR_SPEED
                ret_r = 0
        #if the strophotaxis cycle has not changed:
        else:
            if side==SIDE_LEFT:
            # curve to left
                ret_l = MOTOR_SPEED/1.7
                ret_r = MOTOR_SPEED
            else: # SIDE_RIGHT
            # curve to right
                ret_l = MOTOR_SPEED
                ret_r = MOTOR_SPEED/1.7
...
def behaviour_avoid_line(side):
    ...
    global stro_cycle_change, avoid_line_persist_until
...
def behaviour_strophotaxis(side):
    ...
    global turning_side, COUNT, stro_cycle_change
